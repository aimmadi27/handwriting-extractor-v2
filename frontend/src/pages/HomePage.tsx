...
import { fetchLoginUrl } from '../api/client';
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800 text-white px-4">
      <div className="max-w-md w-full text-center space-y-8">
        ...
        <ul className="text-left space-y-3 text-slate-300">
          {[
            'Works with any document type — forms, assessments, letters, receipts',
            'AI extracts structure: key-value pairs, tables, Q&A, paragraphs',
            'Review and edit inline before exporting',
            'Export to PDF, Word, Excel, or JSON',
          ].map((f) => (
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white text-slate-900 rounded-xl font-semibold text-base shadow hover:bg-slate-100 transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {loading ? (
